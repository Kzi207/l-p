...
export async function POST(request: Request) {
  ...
  if (body.kind === "journal") return notifyPartnerFromDocument({ ...common, collectionName: "journalEntries", senderField: "authorId", type: "journal", route: "/journal", content: (item) => ({ title: `${String(item.authorName || "Người thương")} vừa viết nhật ký ${String(item.mood || "💗")}`, body: String(item.text || "Có một khoảnh khắc mới của hai đứa.") }) });
